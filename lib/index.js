(function (factory, window) {
  ...
  function resetMarker(marker) {
    ...
    // In leaflet 1.9.0 _addFocusListenersOnLayer is introduced wich forces tooltips 
    // to be opened and closed when bound layers receive focus and blur events, regardless
    // if tooltip is configured to be permantent
    marker.getElement().addEventListener('blur', ()=>{
      marker.openTooltip(); //prevent close after click blur
    })
  ...
  function removeAllMarkers() {
    getMarkers().forEach((marker) => marker.remove());
    markerList = [];
  }
  ...
  function getPosition(el) {
    // Failsafe to prevent element null
    if(el) {
  ...
  function setEdgePosition() {
    ...
    for (let i = 0; i < markerList.length; i++) {
      ...
      // In leaflet 1.9.0 _addFocusListenersOnLayer is introduced wich forces tooltips 
      // to be opened and closed when bound layers receive focus and blur events
      // Must update tooltip's offset option to prevent movement when marker' focus event is triggered 
      // on click, which will attempt to open tooltip at the original offset
      let offset = [tooltip.x - marker.x + width, tooltip.y - marker.y + height / 2];
      markerList[i].getTooltip().options.offset = offset;
  ...
  TooltipLayout['resetMapContent'] = resetMapContent;
